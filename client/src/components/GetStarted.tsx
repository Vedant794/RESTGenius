...
export default function GetStarted() {
  ...
  useEffect(() => {
    ...
    const activeOption = customizeOptions.find(
      (option) => option.link === currentPath
    );
    if (activeOption) {
  ...
  return (
    <>
      ...
      <div className="flex h-screen mt-[13vh] w-auto">
        <aside
          className={`side-navBar h-[88vh] w-[16vw] fixed ${mode ? "bg-slate-100" : "bg-[#323939]"} flex flex-col items-center justify-between`}
        >
          <ul className="mb-2">
          ...
          <Link to="/getstarted/previewCustomization">
            <button className="h-auto w-auto px-5 py-2 bg-green-600 mb-8 text-lg font-serif text-white font-semibold rounded-xl">
              Preview
            </button>
          </Link>
        </aside>

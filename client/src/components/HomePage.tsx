...
import openSource from '../assets/open-source-2.png'
...
export default function HomePage() {
  ...
  return (
    <>
    <div className={`upSideHomePage relative h-[55rem] w-full transform transition-all duration-1000 ease-out ${isVisible ? 'opacity-100' : 'opacity-0'}`}>
    <div className="relative bg-cover bg-center h-[51rem] w-full" style={{backgroundImage:`url(${homeBack})` }}>
      <header className='header-api w-full h-[15%] flex justify-evenly items-center'>
        <div className="logo-login flex justify-between w-[85%] items-center">
          <span title='Smart API generation for modern day Programmers'><img src={modelLogo} alt="Logo" className="h-[80px] w-[80px] cursor-pointer"/></span>
      ...
      <section className={`h-[50%] w-full flex flex-col justify-center items-start px-56`}>
        ...
        <div>
        <button className='border-white border-2 w-[13rem] h-[3rem] text-white font-semibold font-sans rounded-full hover:bg-white hover:text-black my-4 mr-3'>Get Started</button>
    ...
    <main className="mainContent h-[80rem] w-full flex flex-col mt-28">
      {mainContentHolder.map((item:mainContentInfo,index:number)=>(
        <div className="contentInfo flex justify-evenly items-center h-[25%]" key={index}>
          {index%2==0 ? (
            <>
          <img src={item.image} alt={item.title} className='h-[90%] w-[30%] bg-cover'/>
